...
//* Ajouter les maisons
  ...
  Future<List<Homes>> getHomes() async {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        List<Homes> homesList =
            jsonData.map((json) => Homes.fromJson(json)).toList();
        print('tous les maisons');
        print(homesList);
        print('la reponse 2');
        return homesList;
      } else {
...
// Importez le modèle de conversation si vous en avez un
  ...
  static Future<void> logout() async {
    ...
    final uri = Uri.parse(
        Logout); // Assurez-vous que votre API a un endpoint de déconnexion
...
//!La Messagerie/////////////////////////////////////////////////////////
  ...
  Future<dynamic> sendMessage(
      String userId, String adminId, String content, bool isAdmin) async {
    //var client = http.Client();

    final response = await http.post(
      Uri.parse(
          '$Api_url/conversations/${isAdmin ? 'admin' : 'user'}/$adminId/${isAdmin ? 'user' : 'admin'}/$userId'),
      body: {'content': content},
    );

    if (response.statusCode == 200) {
      // Utiliser le code 201 pour succès de création
...
//*gg//////////////////////////////////////////////////////////****** */ */
  Future<void> _uploadImages(List<File> images) async {
    final authToken =
        await getAuthToken(); // Assurez-vous d'ajouter le token d'authentification
    final headers = {
      'Content-Type': 'application/json',
      'Authorization': 'Bearer $authToken',
    };
